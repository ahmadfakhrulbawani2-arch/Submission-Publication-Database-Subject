...
SELECT nama_resepsionis, usia, alamat FROM resepsionis 
WHERE usia IN (
	SELECT MIN(usia) FROM resepsionis
...
SELECT -- DISTINCT
	pl.nama_pelanggan, ps.jumlah_kamar, km.nama_kamar -- COUNT(ps.id) AS banyak_order
FROM pelanggan pl 
...
-- GROUP BY pl.nama_pelanggan, ps.jumlah_kamar, km.nama_kamar
-- HAVING pl.nama_pelanggan = 'Daniel Martinez';

-- no 3.

SELECT rs.id, rs.nama_resepsionis, rs.no_telp FROM resepsionis rs
WHERE rs.id IN (
	SELECT ps.resepsionis_id FROM pesanan ps
...
SELECT
    SUM(total_harga_kamar) AS total_semua_order,
    AVG(total_harga_kamar) AS rata_rata_per_order
FROM (
    SELECT 
		SUM(COALESCE(fl.harga_per_fasilitas * kf.jumlah_fasilitas, 0) + DATEDIFF(ps.tanggal_checkout, ps.tanggal_checkin) * km.harga_per_malam) AS total_harga_kamar
	FROM pesanan ps
	INNER JOIN order_kamar ok ON ps.id = ok.order_id
	INNER JOIN kamar km ON ok.kamar_nomor = km.nomor
	LEFT JOIN kamar_fasilitas kf ON km.nomor = kf.kamar_nomor
	LEFT JOIN fasilitas fl ON kf.fasilitas_id = fl.id
	JOIN pembayaran pb ON ps.id = pb.order_id
...
-- 	   SELECT 
--         ps.id AS order_id,
--         SUM(DATEDIFF(ps.tanggal_checkout, ps.tanggal_checkin) * km.harga_per_malam * ps.jumlah_kamar) 
--         + COALESCE(SUM(fl.harga_per_fasilitas * kf.jumlah_fasilitas), 0) AS total_harga_per_order
--     FROM pesanan ps
--     INNER JOIN order_kamar ok ON ps.id = ok.order_id
--     INNER JOIN kamar km ON ok.kamar_nomor = km.nomor
--     LEFT JOIN kamar_fasilitas kf ON km.nomor = kf.kamar_nomor
--     LEFT JOIN fasilitas fl ON kf.fasilitas_id = fl.id
--     INNER JOIN pembayaran pb ON ps.id = pb.order_id
--     WHERE pb.status = 'Lunas'
--     GROUP BY ps.id;
...
SELECT 
	pl.NIK, pl.nama_pelanggan, pl.no_telp, km.nama_kamar, km.nomor, km.tipe_kamar
FROM pembayaran pb
...
-- normalized
SELECT km.nomor AS no_kamar, fl.nama AS nama_fasilitas FROM kamar km
INNER JOIN kamar_fasilitas kf ON km.nomor = kf.kamar_nomor
INNER JOIN fasilitas fl ON kf.fasilitas_id = fl.id
WHERE km.tipe_kamar = 'Deluxe';

-- multi-value
SELECT 
    km.nomor AS no_kamar,
    GROUP_CONCAT(fl.nama SEPARATOR ', ') AS fasilitas
FROM kamar km
INNER JOIN kamar_fasilitas kf ON km.nomor = kf.kamar_nomor
INNER JOIN fasilitas fl ON kf.fasilitas_id = fl.id
WHERE km.tipe_kamar = 'Deluxe'
...
SELECT fl.nama, SUM(kf.jumlah_fasilitas) AS banyak_fasil FROM fasilitas fl
INNER JOIN kamar_fasilitas kf ON fl.id = kf.fasilitas_id
GROUP BY fl.nama
ORDER BY banyak_fasil DESC;

-- no 10

SELECT * FROM pelanggan pl
WHERE pl.NIK LIKE '12%';